...
$platforms = $conn->query("SELECT PlatformID, PlatformName FROM platform");
if (!$platforms) {
    die("Error fetching platforms: " . $conn->error);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert the relationship into the videogame_platform table
   /* $sql = "INSERT INTO videogame_platform (PlatformID, GameID) VALUES (?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $platformID, $gameID);
...
*/
     ...
     $sql = "INSERT INTO runson (PlatformID, GameID)
     VALUES ($platformID, $gameID)";
     
    if ($conn->query($sql) === TRUE) {
        $message = "New platform to videogame link record created successfully!";
        $messageType = "success";
    } else {
...
<?php
    ...
    ?>
        <h2>Enter Link from Videogame to Platform</h2>
        ...
        <select id="platform_id" name="platform_id" required>
            ...
            <?php while ($platform = $platforms->fetch_assoc()) { ?>
                <option value="<?php echo $platform['PlatformID']; ?>">
                    <?php echo htmlspecialchars($platform['PlatformName']); ?>
                </option>
            <?php } ?>
        </select>
        </div>

        <input type="submit" value="Link Platform">
